    ...
    <section class="container mx-auto mt-10 mb-10" x-data="{ tab: 'all' }">
        <div class="">
          <ul class="flex flex-wrap mb-5 justify-center p-2 space-x-1 text-xl font-semibold text-center">
            <li class="">
              <a href="#" @click.prevent="tab = 'all'" :class="{ 'active' : tab === 'all' }" class="bg-gray-100 px-5 py-2 underline hover:bg-orange-300">All</a>
            </li>
            @forelse($categories as $category)
            <li class="">
                <a href="#" @click.prevent="tab = '{{ $category->name }}'" :class="{ 'active' : tab === '{{ $category->name }}' }" class="bg-gray-100 px-5 py-2 underline hover:bg-orange-300">{{ $category->name }}</a>
              </li>
            @empty
            <li class="">
